...
app.delete( '/paises/:pais/:provincia/:ciudad', (_req,_res)=> {
  ...
  if (p){
    ...
    if(!pr){return _res.status(400).send("no bitches?")}
      ...
      const reg = ciudad.registroDeTemperatura.find( (pa)=>{return pa.fecha.toString() === _req.body.fecha } )
